...
export default function SearchPlatforms() {
	...
	const goToSearchByPlatform = (platformId: number) => {
		const query = new URLSearchParams({ platforms: String(platformId) });
					...
					{loading && (
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
							{Array.from({ length: 12 }).map((_, index) => (
								<div key={`platform-skeleton-${index}`} className="h-52 rounded-xl border border-gray-700 bg-black/20 animate-pulse" />
							))}
						</div>
					)}
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
							{platforms.map((platform) => (
								<article
									...
									role="button"
									tabIndex={0}
									onClick={() => goToSearchByPlatform(platform.id)}
									onKeyDown={(event) => {
										if (event.key === "Enter" || event.key === " ") {
											event.preventDefault();
											goToSearchByPlatform(platform.id);
										}
									}}
								>
									<Image
